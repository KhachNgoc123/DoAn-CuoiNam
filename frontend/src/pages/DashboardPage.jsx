...
function trendTooltip(point) {
  ...
  return [
    tooltip.period || point.label,
    `Đang điều trị: ${tooltip.active_patients ?? point.active_patients ?? 0} bệnh nhân`,
    `Đã hoàn thành: ${tooltip.completed_patients ?? point.completed_patients ?? 0} bệnh nhân`,
    `Tổng cộng: ${tooltip.total_patients ?? point.total_patients ?? 0} bệnh nhân`,
  ].join('\n')
}
...
export default function DashboardPage() {
  ...
  const patientTrend = data?.patient_trend || []
  const hasTrendData = patientTrend.some((point) => Number(point.total_patients || 0) > 0)
  const fallbackChartPoints = [
    {
      ...
      tooltip: {
        period: 'Bệnh nhân',
        active_patients: data?.active_medical_records || 0,
        completed_patients: data?.completed_medical_records || 0,
        total_patients: data?.total_patients || 0,
      },
    },
    {
      key: 'records',
      label: 'HS',
      total_patients: data?.total_medical_records || 0,
      tooltip: {
        period: 'Hồ sơ bệnh án',
        active_patients: data?.active_medical_records || 0,
        completed_patients: data?.completed_medical_records || 0,
        total_patients: data?.total_medical_records || 0,
      },
    },
    {
      key: 'prescriptions',
      label: 'Toa',
      total_patients: data?.total_prescriptions || 0,
      tooltip: {
        period: 'Toa thuốc',
        active_patients: data?.active_prescriptions || 0,
        completed_patients: completedPrescriptions,
        total_patients: data?.total_prescriptions || 0,
      },
    },
    {
      key: 'schedules',
      label: 'Lịch',
      total_patients: data?.medicine_schedules || 0,
      tooltip: {
        period: 'Lịch uống thuốc',
        active_patients: data?.active_medicine_schedules || 0,
        completed_patients: data?.paused_medicine_schedules || 0,
        total_patients: data?.medicine_schedules || 0,
      },
    },
  ]
  const chartPoints = hasTrendData ? patientTrend : fallbackChartPoints
  const maxTrendValue = Math.max(...chartPoints.map((point) => Number(point.total_patients || 0)), 0)
